Shader "TK/Material/TKLitToonShader"
{
    Properties
    {
        ...
        [Header(Add)]
        [Space(5)]
        _AddMap1("Add Map1", 2D) = "black" { }
        _AddFactor1("Add Factor1", range(0.0, 1.0)) = 1.0
        [HDR]_AddColor1("Emission Color1", color) = (0, 0, 0, 0)
        [Space(30)]
    ...
    SubShader
    {
        ...
        TEXTURE2D(_AddMap1); SAMPLER(sampler_AddMap1);
        TEXTURE2D(_AddMap2); SAMPLER(sampler_AddMap2);
        TEXTURE2D(_AddMap3); SAMPLER(sampler_AddMap3);
...
# ifdef _MAIN_LIGHT_SHADOWS
                ...
                half4 AddColor1 = SAMPLE_TEXTURE2D(_AddMap1, sampler_AddMap1, input.uv.xy) * _AddColor1 * (_AddFactor1 / 100);
        
                FinalColor += AddColor1;
