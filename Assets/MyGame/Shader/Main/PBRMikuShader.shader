Shader "TK/PBR/MMDModel/PBRMikuShader"
{
Properties
    {
        [Header(BaseColor)]
        ...
        [Header(Matcap)]
        [Toggle(_MATCAP_SWITCH)]_MATCAP_SWITCH("_MATCAP?", Float) = 1
    ...
    #pragma multi_compile _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS _ADDITIONAL_ON
    ...
    #pragma shader_feature _MATCAP_SWITCH 
    ...
    half4 fragAllWork(VaryingsPBR IN, bool shouldOnlyDoAlphaClipAndEarlyExit = false, bool isExtraCustomPass = false)
    {
        ...
        half4 matCap = SAMPLE_TEXTURE2D(_MatcapTex, sampler_MatcapTex, uv);
        finalColor += half4(matCap.rgb * _MatcapPower, 0);
#endif

#if _RIM
        float2 uv2 = TRANSFORM_TEX(IN.uv, _RimTex);
        half4 rim = SAMPLE_TEXTURE2D(_RimTex, sampler_RimTex, uv2) * _RimPower;
        float rimp = 1 - saturate(dot(viewDirectionWS, N)) * _RimPower;
        finalColor.rgb += rim.rgb * pow(abs(rimp), 2.5);
#endif
    return finalColor;
    }
