...
namespace TK.PhysicsBone
{
    ...
    public class PhysicsSoftBone : MonoBehaviour
    {
        ...
        private class Bone
        {
            ...
            public void Inflate(float baseRadius, AnimationCurve radiusCurve, PhysicsBoneSoftBoneMaterial material)
            {
                radius = radiusCurve.Evaluate(normalizedLength) * baseRadius;
        ...
        public List<Transform> m_RootBones;
        ...
        [SerializeField]//アクセス用に一旦Public可してる
        public PhysicsBoneSoftBoneMaterial m_Material;
        private PhysicsBoneSoftBoneMaterial m_InstanceMaterial;
        public PhysicsBoneSoftBoneMaterial sharedMaterial
        {
            get
            {
                if (m_Material == null)
                    m_Material = PhysicsBoneSoftBoneMaterial.defaultMaterial;
                return m_Material;
            }
            set
            {
        ...
        public PhysicsBoneSoftBoneMaterial material
        {
            get
            {
        ...
        [SerializeField]//アクセス用に一旦Public可してる
        public List<Collider> m_ExtraColliders = new List<Collider>();
        public List<Collider> extraColliders { get { return m_ExtraColliders; } }
        [SerializeField]
        private float m_Radius = 0;
        public float radius { get { return m_Radius; } set { m_Radius = value; } }
        [SerializeField, PhysicsBoneCurveRect(0, 0, 1, 1)] //アクセス用に一旦Public可してる
        ...
        #region Force
        [SerializeField]
        private PhysicsBoneForceField m_ForceModule;
        public PhysicsBoneForceField forceModule { get { return m_ForceModule; } set { m_ForceModule = value; } }
        [SerializeField]
        ...
        private void UpdateBones(Bone bone, float deltaTime)
        {
            if (bone.depth > startDepth)
            {
                ...
                if (bone.radius > 0)
                {
                    foreach (PhysicsBoneColliderBase collider in PhysicsBoneColliderBase.EnabledColliders)
                    {
                    ...
                    foreach (Collider collider in extraColliders)
                    {
                        if (bone.transform != collider.transform && collider.enabled)
                            PhysicsBoneUtility.PointOutsideCollider(ref newWorldPosition, collider, bone.radius);
                    }
                }
