...
{
    public class PhysicsBoneColliderCylinder : PhysicsBoneColliderBase
    {
        [SerializeField]
        ...
        {
            if (insideMode) PhysicsBoneUtility.PointInsideCylinder(ref position, transform, spacing + margin);
            else PhysicsBoneUtility.PointOutsideCylinder(ref position, transform, spacing + margin);
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            float radius, height;
            PhysicsBoneUtility.GetCylinderParams(transform, out center, out direction, out radius, out height);
