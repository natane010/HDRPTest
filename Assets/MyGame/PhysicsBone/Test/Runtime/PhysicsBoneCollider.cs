...
namespace TK.PhysicsBone
{
    ...
    public class PhysicsBoneCollider : PhysicsBoneColliderBase
    {
        [SerializeField]
        ...
        public override void Collide(ref Vector3 position, float spacing)
        {
            if (referenceCollider is SphereCollider)
            {
                ...
                if (insideMode) PhysicsBoneUtility.PointInsideSphere(ref position, collider, spacing + margin);
                else PhysicsBoneUtility.PointOutsideSphere(ref position, collider, spacing + margin);
            }
            else if (referenceCollider is CapsuleCollider)
            {
                CapsuleCollider collider = referenceCollider as CapsuleCollider;
                if (insideMode) PhysicsBoneUtility.PointInsideCapsule(ref position, collider, spacing + margin);
                else PhysicsBoneUtility.PointOutsideCapsule(ref position, collider, spacing + margin);
            }
            else if (referenceCollider is BoxCollider)
            {
                BoxCollider collider = referenceCollider as BoxCollider;
                if (insideMode) PhysicsBoneUtility.PointInsideBox(ref position, collider, spacing + margin);
                else PhysicsBoneUtility.PointOutsideBox(ref position, collider, spacing + margin);
            }
            else if (referenceCollider is MeshCollider)
            {
                ...
                PhysicsBoneUtility.PointOutsideCollider(ref position, referenceCollider, spacing + margin);
            }
        }
